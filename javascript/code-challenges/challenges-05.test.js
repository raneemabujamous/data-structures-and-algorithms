...
const toLastNames = (people) => {
  let peoples = people.reduce((accumulator, person, idx) => {
    accumulator[idx] = person.firstName + " " + person.lastName;
    return accumulator;
  }, []);
  return peoples;
};
...
const addPurchases = (arr) => {
  return arr.reduce((accumulator, val) => {
    return (accumulator += val.purchasePrice);
  }, 0);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const returnNames = (arr) => {
  let newArr = [];
  newArr = arr.reduce((accumulator, val) => {
