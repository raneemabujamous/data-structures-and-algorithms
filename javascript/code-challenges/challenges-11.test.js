...
const divisibleByFiveTwoToThePower = (input) => {
  let newarr = [];
  newarr = input.map((val) =>
    val.filter((val2) => typeof val2 === "number" && val2 % 5 === 0)
  );
  return newarr.map((res1) => res1.map((res2) => Math.pow(2, res2)));
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findMaleAndFemale = (data) => {
  let newdata = [];
  newdata = data.filter((d) => d.gender === "female" || d.gender === "male");
  return newdata.map((e) => e.name).join(" and ");
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
let findShortest = (data) => {
  let short = data.reduce((val1, val2) =>
    Number(val1.height) < Number(val2.height) ? val1 : val2
  );
  return short.name;
};
/* ------------------------------------------------------------------------------------------------
